...
library(tidyverse)
data.frame(`Year` = c(2018, 2019), `Source` = c("https://www.pewresearch.org/internet/dataset/jan-3-10-2018-core-trends-survey/",
...
```{r}
df2018 <- read_csv("data/raw/core_trends_2018.csv")
df2019 <- read_csv("data/raw/core_trends_2019.csv")
```
...
```{r}
sum2018 <- DataExplorer::introduce(df2018)
sum2018$year = 2018

sum2019 <- DataExplorer::introduce(df2019)
sum2019$year = 2019

order <- names(sum2018)[1:8]

rbind(sum2018, sum2019) %>% 
  tbl_df() %>%
  gather(Property, value, -year) %>%
  spread(year, value) %>%
  filter(Property != "memory_usage", 
         Property != "discrete_columns", 
         Property != "continuous_columns") %>%
  slice(match(order, Property)) %>%
  knitr::kable()
```
